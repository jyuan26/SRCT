...
from data import DIV2Kprism, prismval
import utils
...
def valid(scale):
    ...
    for batch in testing_data_loader:
        ...
        with torch.no_grad():
            #pre = model(lr_tensor)
            pre = forward_chop(model, lr_tensor, scale, 2)#model(lr_tensor)
...
def save_checkpoint(epoch):
    model_folder = "experiment/checkpoint_ESRTprism_x{}/".format(args.scale)
    model_out_path = model_folder + "prism epoch_{}.pth".format(epoch)
